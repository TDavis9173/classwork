...
public class Searching {
	...
	private void proceedWithSearch() {
		...
		int target = -1;
		while(target <= 0 || target >= numbers[numbers.length - 1])
			try {
		...
		System.out.println("\nGet ready to race the computer."
...
//		int index = search(numbers, target);
	...
	public static int binarySearch(int[] searchThis, int startIndex, int endIndex, int target) {
		...
		if(startIndex > endIndex)
			...
			if(searchThis[middleIndex] == target)
				return middleIndex;
			else if(searchThis[middleIndex] > target)
				return binarySearch(searchThis, startIndex, middleIndex - 1, target);
			else
				return binarySearch(searchThis, middleIndex + 1, endIndex, target);
		}
	}
